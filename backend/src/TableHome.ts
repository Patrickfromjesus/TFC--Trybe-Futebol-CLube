...
export default class TableHome extends ClassificationTable {
  ...
  addGoalsFavor = (
    ...
    this.classification[teamHome[0]].goalsFavor += homeTeamGoals[0];
  };
  ...
  handleMatches = (matches: IFinishedMatch[]) => {
    matches.forEach(({ homeTeamGoals, awayTeamGoals, homeTeamId, awayTeamId }) => {
      ...
      this.addGames([homeTeamId]);
      this.addGoalsOwn([homeTeamId], [awayTeamGoals]);
      this.addGoalsFavor([homeTeamId], [homeTeamGoals]);
